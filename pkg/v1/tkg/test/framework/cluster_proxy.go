...
func (p *ClusterProxy) GetPackages(ctx context.Context, namespace string) []*kapppkgv1alpha1.Package {
	c := p.GetClient()
	packagesList := &kapppkgv1alpha1.PackageList{}
	listOptions := &client.ListOptions{
		Namespace: namespace,
	}
	err := c.List(ctx, packagesList, listOptions)
	Expect(err).ToNot(HaveOccurred())
	packages := make([]*kapppkgv1alpha1.Package, len(packagesList.Items))
