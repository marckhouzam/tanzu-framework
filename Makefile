...
build-plugin-admin-%:
	...
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=${DISCOVERY_TYPE}'" --tags "${BUILD_TAGS}" --path $(ROOT_DIR)/cmd/cli/plugin-admin --artifacts $(ARTIFACTS_ADMIN_DIR)/${OS}/${ARCH}/cli --target ${OS}_${ARCH})
...
build-cli-%: prep-build-cli
	...
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=${DISCOVERY_TYPE}'" --tags "${BUILD_TAGS}" --corepath "$(ROOT_DIR)/cmd/cli/tanzu" --artifacts $(ARTIFACTS_DIR)/${OS}/${ARCH}/cli --target  ${OS}_${ARCH})
...
build-cli-local: configure-buildtags-embedproviders build-cli-local-${GOHOSTOS}-${GOHOSTARCH} publish-plugins-local ## Build Tanzu CLI with local standalone discovery. cluster and management-cluster plugins are built with embedded providers.
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=local'" --tags "${BUILD_TAGS}" --path $(ROOT_DIR)/cmd/cli/plugin-admin --artifacts $(ARTIFACTS_ADMIN_DIR)/${GOHOSTOS}/${GOHOSTARCH}/cli --target local)
...
build-cli-mocks: ## Build Tanzu CLI mocks
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile $(addprefix --target ,$(subst -,_,${ENVS})) --version 0.0.1 --ldflags "$(LD_FLAGS)" --tags "${BUILD_TAGS}" --path $(ROOT_DIR)/test/cli/mock/plugin-old --artifacts $(ROOT_DIR)/test/cli/mock/artifacts-old)
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile $(addprefix --target ,$(subst -,_,${ENVS})) --version 0.0.2 --ldflags "$(LD_FLAGS)" --tags "${BUILD_TAGS}" --path $(ROOT_DIR)/test/cli/mock/plugin-new --artifacts $(ROOT_DIR)/test/cli/mock/artifacts-new)
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile $(addprefix --target ,$(subst -,_,${ENVS})) --version 0.0.3 --ldflags "$(LD_FLAGS)" --tags "${BUILD_TAGS}" --path $(ROOT_DIR)/test/cli/mock/plugin-alt --artifacts $(ROOT_DIR)/test/cli/mock/artifacts-alt)
...
release-%: ## Create release for a platform
	...
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=oci'" --tags "${BUILD_TAGS}" --path $(ROOT_DIR)/cmd/cli/plugin-admin --artifacts $(ARTIFACTS_ADMIN_DIR)/${OS}/${ARCH}/cli --target ${OS}_${ARCH})
	$(ROOT_DIR)/hack/embed-pinniped-binary.sh go ${OS} ${ARCH} ${PINNIPED_VERSIONS}
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=oci'" --tags "${BUILD_TAGS}" --corepath "$(ROOT_DIR)/cmd/cli/tanzu" --artifacts $(ARTIFACTS_DIR)/${OS}/${ARCH}/cli --target  ${OS}_${ARCH})
